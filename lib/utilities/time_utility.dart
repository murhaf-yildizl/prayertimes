...
print("DEFER ${deferences}");
 int nearestIndex=deferences.indexOf(deferences.reduce((a,b) => a.inMilliseconds<b.inMilliseconds?a:b));
 ...
 if(remainingTime.inMinutes<-2 )
  {
   if(++nearestIndex>5)
     {
      nearestIndex=0;
      remainingTime=times[nearestIndex].add(Duration(days: 1)).difference(DateTime.now());
     }
 else   remainingTime=times[nearestIndex].difference(DateTime.now());
 ...
 //print(">>>>>>${prayer_times[nearestIndex].name} ${remainingTime.inHours} ${remainingTime.inMinutes.remainder(60)}  ${remainingTime.inSeconds.remainder(60)}" );
  ...
  return{
   ...
   'remainingminutes': remainingTime.inMinutes.remainder(60)<0?0:remainingTime.inMinutes.remainder(60),
   'remainingsecond':remainingTime.inSeconds.remainder(60)<0?0:remainingTime.inSeconds.remainder(60)
  };
